...
export const contentSchemas = {
  hero: z.object({ settings: z.object({}), items: z.array(heroItem).max(100).refine(items => new Set(items.map(item => item.id)).size === items.length, "Book IDs must be unique") }),
...
export const contentDefaults = {
  hero: { settings: {}, items: HERO_BOOKS.map(book => Object.fromEntries(Object.keys(HERO_ITEM_DEFAULT).map(key => [key, String(book[key as keyof typeof book] ?? HERO_ITEM_DEFAULT[key as keyof typeof HERO_ITEM_DEFAULT])]))) },
