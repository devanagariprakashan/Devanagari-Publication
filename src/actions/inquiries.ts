...
export async function submitInquiry(input: { name: string; phone: string; email?: string; subject?: string; message: string }): Promise<ActionResult> {
  ...
  if (!name || !phone || !message) return { error: "Name, phone, and message are required" }
  if (email && !/^\S+@\S+\.\S+$/.test(email)) return { error: "Enter a valid email address" }
  const supabase = await createClient()
  const { error } = await supabase.from('inquiries').insert({ id: crypto.randomUUID(), name, phone, email, subject, message, status: 'unread' })
